...
class ArchiveSpec extends BaseSpec with CirceEq {
    ...
    "create resources" taggedAs ArchivesTag in {
      eventually {
        for {
          ...
          _ <- cl.put[Json](s"/resources/$fullId/_/test-resource:1", payload1, Tweety) { (_, response) =>
            response.status shouldEqual StatusCodes.Created
          }
          _ <- cl.put[Json](s"/resources/$fullId2/_/test-resource:2", payload2, Tweety) { (_, response) =>
            response.status shouldEqual StatusCodes.Created
          }
        } yield succeed
      }
  ...
  "creating archives" should {
    "succeed" taggedAs ArchivesTag in {
      ...
      cl.put[Json](s"/archives/$fullId/test-resource:archive", payload, Tweety) { (_, response) =>
        response.status shouldEqual StatusCodes.Created
      }
    }
    ...
    "failed if payload is wrong" taggedAs ArchivesTag in {
      ...
      cl.put[Json](s"/archives/$fullId/archive2", payload, Tweety) { (json, response) =>
  ...
  "fetching archive" should {
    "succeed returning metadata" taggedAs ArchivesTag in {
      cl.get[Json](s"/archives/$fullId/test-resource:archive", Tweety) { (json, response) =>
    ...
    "succeed returning binary" taggedAs ArchivesTag in {
      ...
      cl.get[ByteString](s"/archives/$fullId/test-resource:archive", Tweety, acceptAll) { (byteString, response) =>
    ...
    "failed when a resource in the archive cannot be fetched" taggedAs ArchivesTag in {
      cl.get[Json](s"/archives/$fullId/test-resource:archive", Tweety, acceptAll) { (json, response) =>
