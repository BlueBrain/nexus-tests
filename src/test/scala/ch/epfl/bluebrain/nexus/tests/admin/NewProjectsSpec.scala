...
class NewProjectsSpec extends NewBaseSpec {
  ...
  "projects API" should {
    ...
    val base        = s"resources/$id/_/"
    val vocab       = s"vocabs/$id/"
    ...
    "update project and fetch revisions"  taggedAs ProjectsTag in {
      ...
      val baseRev2  = s"${genString()}/"
      val vocabRev2 = s"${genString()}/"
      ...
      val baseRev3  = s"${genString()}/"
      val vocabRev3 = s"${genString()}/"
