  ...
  s"uploading an attachment against the $storageType storage" should {
    ...
    "fetch attachment metadata" taggedAs StorageTag in {
      val expected = jsonContentOf(
        ...
        replacements(
          Coyote,
          quote("{id}")        -> s"${config.deltaUri}/resources/$fullId/_/attachment.json",
          quote("{filename}")  -> s"attachment.json",
          quote("{storageId}") -> s"nxv:$storageName",
          quote("{projId}")    -> s"$fullId",
          quote("{project}")   -> s"${config.deltaUri}/projects/$fullId"
        )
      )
