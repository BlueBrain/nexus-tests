...
class ResourcesSpec extends BaseSpec with Eventually with Inspectors with CancelAfterFailure {
  ...
  "creating projects" should {
    ...
    "succeed if payload is correct" in {
      ...
      cl(Req(PUT, s"$adminBase/projects/$id1", headerUser, kgProjectReqEntity(name = id1)))
        .mapResp(_.status shouldEqual StatusCodes.Created)
      cl(Req(PUT, s"$adminBase/projects/$id2", headerUser, kgProjectReqEntity(name = id2)))
        .mapResp(_.status shouldEqual StatusCodes.Created)
    }
  }
  ...
  "adding schema" should {
    "create a schema" in {
      ...
      eventually {
        cl(Req(PUT, s"$kgBase/schemas/$id1/test-schema", headerUser, schemaPayload.toEntity))
          .mapResp(_.status shouldEqual StatusCodes.Created)
      }
    }
    "creating a schema with property shape" in {
      ...
      cl(Req(POST, s"$kgBase/schemas/$id1", headerUser, schemaPayload.toEntity))
        .mapResp(_.status shouldEqual StatusCodes.Created)
    }
  ...
  "creating a resource" should {
    "succeed if the payload is correct" in {
      ...
      cl(Req(PUT, s"$kgBase/resources/$id1/test-schema/test-resource:1", headerUser, payload.toEntity))
        .mapResp(_.status shouldEqual StatusCodes.Created)
      ...
      cl(Req(PUT, s"$kgBase/resources/$id1/${id2}/test-resource:10", headerUser, payload2.toEntity))
        .mapResp(_.status shouldEqual StatusCodes.Created)
    }

    "fetch the payload" in {
      cl(Req(GET, s"$kgBase/resources/$id1/test-schema/test-resource:1", headerUser)).mapJson { (json, result) =>
  ...
  "cross-project resolvers" should {
    ...
    "fail if the schema doesn't exist in the project" in {
      ...
      cl(Req(PUT, s"$kgBase/resources/$id2/test-schema/test-resource:1", headerUser, payload.toEntity))
    ...
    "fail to create a cross-project-resolver for proj2 if identities are missing" in {
      ...
      cl(Req(POST, s"$kgBase/resolvers/$id2", headerUser, resolverPayload.removeField("identities").toEntity))
        .mapResp(_.status shouldEqual StatusCodes.BadRequest)
    }

    "create a cross-project-resolver for proj2" in {

      cl(Req(POST, s"$kgBase/resolvers/$id2", headerUser, resolverPayload.toEntity))
        .mapResp(_.status shouldEqual StatusCodes.Created)
    }
    ...
    "update a cross-project-resolver for proj2" in {
      ...
      eventually {
        cl(Req(PUT, s"$kgBase/resolvers/$id2/example-id?rev=1", headerUser, updated.toEntity))
    ...
    "fetch the update" in {
      ...
      cl(Req(GET, s"$kgBase/resolvers/$id2/example-id", headerUser)).mapJson { (json, result) =>
        result.status shouldEqual StatusCodes.OK
    ...
    "wait for the cross-project resolver to be indexed" in {
      ...
      eventually {
        cl(Req(GET, s"$kgBase/resolvers/$id2", headerUser)).mapJson { (json, result) =>
          result.status shouldEqual StatusCodes.OK
  ...
  "updating a resource" should {
    "send the update" in {
      ...
      cl(Req(PUT, s"$kgBase/resources/$id1/test-schema/test-resource:1?rev=1", headerUser, payload.toEntity))
        .mapResp(_.status shouldEqual StatusCodes.OK)
    }
    "fetch the update" in {
      val expected = jsonContentOf(
      ...
      forAll(List(s"$kgBase/resources/$id1/test-schema", s"$kgBase/resources/$id1/_")) { base =>
        cl(Req(GET, s"$base/test-resource:1", headerUser)).mapJson { (json, result) =>
          result.status shouldEqual StatusCodes.OK
    ...
    "fetch previous revision" in {
      ...
      forAll(List(s"$kgBase/resources/$id1/test-schema", s"$kgBase/resources/$id1/_")) { base =>
        cl(Req(GET, s"$base/test-resource:1?rev=1", headerUser)).mapJson { (json, result) =>
          result.status shouldEqual StatusCodes.OK
          json.removeField("_createdAt").removeField("_updatedAt") should equalIgnoreArrayOrder(expected)
        }
      }
  ...
  "tagging a resource" should {
    ...
    "create a tag" in {
      ...
      cl(Req(POST, s"$kgBase/resources/$id1/test-schema/test-resource:1/tags?rev=2", headerUser, tag1.toEntity))
        .mapResp(_.status shouldEqual StatusCodes.Created)
      cl(Req(POST, s"$kgBase/resources/$id1/_/test-resource:1/tags?rev=3", headerUser, tag2.toEntity))
        .mapResp(_.status shouldEqual StatusCodes.Created)
    }
    ...
    "fetch a tagged value" in {
      ...
      cl(Req(GET, s"$kgBase/resources/$id1/test-schema/test-resource:1?tag=v1.0.1", headerUser)).mapJson {
        (json, result) =>
          result.status shouldEqual StatusCodes.OK
      ...
      cl(Req(GET, s"$kgBase/resources/$id1/_/test-resource:1?tag=v1.0.0", headerUser)).mapJson { (json, result) =>
        result.status shouldEqual StatusCodes.OK
  ...
  "listing resources" should {
    ...
    "list default resources" in {
      ...
      forAll(resources) {
        case (segment, expected) =>
          eventually {
            cl(Req(GET, s"$kgBase/$segment/$id1", headerUser)).mapJson { (json, result) =>
              result.status shouldEqual StatusCodes.OK
              removeSearchMetadata(json) shouldEqual expected
            }
          }
    ...
    "add more resource to the project" in {
      ...
      forAll(2 to 5) { resourceId =>
        ...
        cl(Req(PUT, s"$kgBase/resources/$id1/test-schema/test-resource:$resourceId", headerUser, payload.toEntity))
          .mapResp(_.status shouldEqual StatusCodes.Created)
      }
    ...
    "list the resources" in {
      ...
      eventually {
        cl(Req(GET, s"$kgBase/resources/$id1/test-schema", headerUser)).mapJson { (json, result) =>
          result.status shouldEqual StatusCodes.OK
          removeSearchMetadata(json) shouldEqual expected
        }
      }
