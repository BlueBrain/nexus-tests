...
class OrgsSpec extends BaseSpec with OptionValues with CancelAfterFailure with Eventually {
  ...
  "creating an organization" should {
    ...
    "fail if the permissions are missing" in {
      cl(Req(PUT, s"$adminBase/orgs/${genId()}", headerUser, Json.obj().toEntity)).mapJson { (json, result) =>
    ...
    "succeed if payload is correct" in {
      cl(Req(PUT, s"$adminBase/orgs/$id", headerUser, orgReqEntity(id))).mapJson { (json, result) =>
  ...
  "fetching an organization" should {
    ...
    "fail if the permissions are missing" in {
      ...
      cl(Req(PUT, s"$adminBase/orgs/$id", headerUser, create)).mapJson { (json, result) =>
        result.status shouldEqual StatusCodes.Created
        json.removeMetadata() shouldEqual createRespJson(id, 1L, "orgs", "Organization")
      }

      cleanAcls
      cl(Req(uri = s"$adminBase/orgs/$id", headers = headerUser)).mapJson { (json, result) =>
        result.status shouldEqual StatusCodes.Forbidden
    ...
    "return not found when fetching a non existing revision of an organizations" in {
      cl(Req(uri = s"$adminBase/orgs/$id?rev=3", headers = headerUser)).mapResp { result =>
    ...
    "return not found when fetching a non existing organization" in {
      cl(Req(uri = s"$adminBase/orgs/$nonExistent", headers = headerUser)).mapResp { result =>
        result.status shouldEqual StatusCodes.NotFound
      }
    }
  }
  ...
  "updating an organization" should {
    ...
    "fail if the permissions are missing" in {
      cleanAcls
      cl(Req(PUT, s"$adminBase/orgs/$id?rev=1", headerUser, create)).mapJson { (json, result) =>
        result.status shouldEqual StatusCodes.Forbidden
        json shouldEqual jsonContentOf("/iam/errors/unauthorized-access.json", errorCtx)
      }
    ...
    "fail when wrong revision is provided" in {
      cl(Req(PUT, s"$adminBase/orgs/$id?rev=4", headerUser, create)).mapJson { (json, result) =>
        result.status shouldEqual StatusCodes.Conflict
    ...
    "fail when organization does not exist" in {
      cl(Req(PUT, s"$adminBase/orgs/$nonExistent?rev=1", headerUser, create)).mapJson { (json, result) =>
        result.status shouldEqual StatusCodes.NotFound
    ...
    "succeed and fetch revisions" in {
      ...
      cl(Req(PUT, s"$adminBase/orgs/$id?rev=1", headerUser, update)).mapJson { (json, result) =>
        result.status shouldEqual StatusCodes.OK
      ...
      cl(Req(PUT, s"$adminBase/orgs/$id?rev=2", headerUser, update2)).mapJson { (json, result) =>
        result.status shouldEqual StatusCodes.OK
      ...
      cl(Req(uri = s"$adminBase/orgs/$id?rev=3", headers = headerUser)).mapJson { (json, result) =>
        result.status shouldEqual StatusCodes.OK
        validateAdminResource(json, "Organization", "orgs", id, updatedName2, 3L, id)

      }

      cl(Req(uri = s"$adminBase/orgs/$id?rev=2", headers = headerUser)).mapJson { (json, result) =>
        result.status shouldEqual StatusCodes.OK
        validateAdminResource(json, "Organization", "orgs", id, updatedName, 2L, id)

      }

      cl(Req(uri = s"$adminBase/orgs/$id?rev=1", headers = headerUser)).mapJson { (json, result) =>
        result.status shouldEqual StatusCodes.OK
        validateAdminResource(json, "Organization", "orgs", id, s"$id organization", 1L, id)
      }
    }
  ...
  "deprecating an organization" should {
    ...
    "fail when wrong revision is provided" in {
      ...
      cl(Req(DELETE, s"$adminBase/orgs/$id?rev=4", headerUser)).mapJson { (json, result) =>
        result.status shouldEqual StatusCodes.Conflict
    ...
    "fail when revision is not provided" in {
      cl(Req(DELETE, s"$adminBase/orgs/$id", headerUser)).mapJson { (json, result) =>
    ...
    "succeed if organization exists" in {
      cl(Req(DELETE, s"$adminBase/orgs/$id?rev=1", headerUser)).mapJson { (json, result) =>
        result.status shouldEqual StatusCodes.OK
